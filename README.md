...
如果你只想安装当前使用的 agent 适配层，也可以：

```bash
bash scripts/install-agent-adapters.sh                 # 自动识别 Codex / Claude Code
bash scripts/install-agent-adapters.sh --agent codex  # 只装 Codex
bash scripts/install-agent-adapters.sh --agent claude # 只装 Claude Code
```

> 安装脚本会把适配器**复制**到目标目录，而不是建立指向源码仓库的符号链接；因此即使后续删除 clone 下来的仓库，本机已安装的 skill / command / launcher 仍然可用。
...
- 自动识别当前环境里已存在的 **Codex** / **Claude Code** 配置目录
- 对 Codex：安装到 `${CODEX_HOME:-$HOME/.codex}/skills/`
- 对 Claude Code：安装到 `${CLAUDE_DIR:-$HOME/.claude/commands}/`
- 如需显式指定：`--agent codex|claude|all`
